param([String]$OutputFolder=$null,[String]$ExtensionId=$null,[Switch]$Remove, [Switch]$WhatIf) 
 ...
 ##: Array to store extension names 
 $extensionNames = @()
 ...
                 } 
             } else { 
                 ##: Add only the name to the array if ExtensionId is provided and matches
                 $extensionNames += $name
 ...
         } else { 
             ##: Add only the name to the array if no ExtensionId is provided
             $extensionNames += $name
         } 
 ...
 ##: Output the extension names as a comma-separated list 
 if ($extensionNames.Count -gt 0) {
