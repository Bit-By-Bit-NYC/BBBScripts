...
# - Always prints SKU menu (1–7) before prompting
# - Prompts connection rename(s) up-front (before any destructive work)
# - Supports AZ (zone-redundant) / non-AZ, Active-Active (2 PIPs)
# - Zonal PIP compatibility guard (offers -nozone)
# - Backs up connection JSON + PSKs; restores IPsec policies (with --sa-max-size)
...
select_final_sku_and_aa(){
  ...
  if [[ "${SKIP_SKU_PROMPT,,}" == "true" ]]; then
    ...
    if [[ "${SKIP_AA_PROMPT,,}" == "true" ]]; then
      ...
      log "Using FINAL_SKU=$FINAL_SKU, Active-Active=$ACTIVE_ACTIVE (inherited; prompts skipped)."
    ...
    log "Using FINAL_SKU=$FINAL_SKU, Active-Active=$ACTIVE_ACTIVE (SKU inherited)."
    echo "$FINAL_SKU"; return
  fi
...
# Writes a mapping file: $PLAN_FILE with lines "OLD<TAB>NEW"
collect_connection_rename_plan_from_live(){
  ...
  while read -r C; do
    [ -z "$C" ] && continue
    local NEWC="$C"
    read_default NEWC "  New name for '$C'" "$C"
    printf "%s\t%s\n" "$C" "$NEWC" >> "$plan"
  done <<< "$CONNS"
}
...
lookup_new_conn_name(){
  # usage: lookup_new_conn_name PLAN_FILE OLD_NAME -> echoes NEW or OLD if not found
  local plan="$1" old="$2"
  local line new
  line=$(grep -F "$old"$'\t' "$plan" 2>/dev/null | head -n1)
  if [ -n "$line" ]; then new="${line#*$'\t'}"; echo "$new"; else echo "$old"; fi
}
...
# -------- Option 2: Backup + Recreate current (all prompts up-front) --------
if [ "$CHOICE" = "2" ]; then
  ...
  # Up-front rename plan for existing connections
  collect_connection_rename_plan_from_live "$PLAN_FILE"

  # PIPs (pre-create/validate up-front)
  ...
  echo "This will delete connections, delete the gateway, create $NEW_GW with SKU=$FINAL_SKU and NEW_PIP(s), then recreate connections per your rename plan."
...
# -------- Option 3: Restore GW + connections from previous backup (all prompts up-front) --------
if [ "$CHOICE" = "3" ]; then
  build_valid_backup_list
  ...
  # Up-front rename plan from backup
...
# -------- Option 4: Restore connections only (all prompts up-front) --------
if [ "$CHOICE" = "4" ]; then
  ...
  # Up-front rename plan
  collect_connection_rename_plan_from_backup "$PLAN_FILE" "$RESTORE_SRC"
fi

# -------- Recreate connections from RESTORE_SRC or BACKUP_DIR --------
...
# Apply connection creations (use rename plan if present)
if [ -d "$SRC/conn-json" ]; then
  ...
  for CJ in "$SRC"/conn-json/*.json; do
    ...
    # Apply IPsec policies — CLI 2.76.0 requires --sa-max-size; default 100MB if 0
